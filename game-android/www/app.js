let fileSystem = {
    ...
    "style.css": `body {\n  background: #1e1e24;\n  color: #fff;\n  text-align: center;\n  padding-top: 50px;\n}\nh1 { color: #61afef; }\nbutton { padding: 10px; background: #98c379; border: none; border-radius: 5px; cursor: pointer; color: #1e2227; font-weight: bold; }`,
...
function renderFileTree() {
    ...
    Object.keys(fileSystem).forEach(file => {
        ...
        div.className = 'file-item ' + (file === activeFile ? 'active' : '');
        div.innerText = '📄 ' + file; 
...
function renderTabs() {
    ...
    openTabs.forEach(file => {
        const div = document.createElement('div'); 
        div.className = 'tab ' + (file === activeFile ? 'active' : '');
...
function togglePreview() {
    ...
    if (container.style.display === 'flex') { 
        ...
        let cssCode = '<style>' + (fileSystem["style.css"] || "") + '</style>';
        let jsCode = '<script>' + (fileSystem["app.js"] || "") + '<\/script>';
        
        let finalSource = htmlCode.replace('</head>', cssCode + '</head>').replace('</body>', jsCode + '</body>');
        container.style.display = 'flex';
        const dst = iframe.contentDocument || iframe.contentWindow.document; 
        dst.open(); 
        dst.write(finalSource); 
        dst.close();
    }
}

editor.value = fileSystem[activeFile]; 
renderFileTree(); 
renderTabs();
